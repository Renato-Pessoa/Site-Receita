...
def criaReceita(request):
    if request.method == 'POST':
        ...
        preparo = request.POST['modo_preparo']
        tempo_preparo = request.POST['tempo_preparo']
        rendimento = request.POST['rendimento']
        categoria_receita = request.POST['categoria']
        foto_receita = request.FILES['foto_receita']
        user = get_object_or_404(User, pk=request.user.id)
        receita = Receita.objects.create(pessoa=user, nome_receita=nome_receita, 
        ingredientes=ingredientes, modo_preparo=preparo, 
        tempo_preparo=tempo_preparo, rendimento=rendimento, 
        categoria=categoria_receita, foto_receita=foto_receita)
